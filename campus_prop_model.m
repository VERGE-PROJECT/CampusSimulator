function L = campus_prop_model(d,model,f,hUT,hBS,hE,NLOS_condition,indoor_condition,d2Dindoor)
...
switch model
    ...
    case 1  %UMa model
        ...
        % Compute hE. Subject to hUT and d
        g = (5/4) * (d/100)^3 * exp(-d/150);
        C = ((hUT -13)/10)^1.5 * g;
        ...
        %First compute path loss under LOS conditions:
        if d<=dBP
            L_LOS=28+22*log10(d3D)+20*log10(f);
        else
            L_LOS=28+40*log10(d3D)+20*log10(f)-9*log10(dBP*dBP+(hBS-hUT)*(hBS-hUT));
        end
